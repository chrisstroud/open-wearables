...
<Note>
  **Pull mode, late revisions, and cursors.** Every live pull captures one fixed end time before provider work begins and re-fetches a trailing window from the connection's last successful cursor. The default `PULL_SYNC_LOOKBACK` is `2d`; it accepts compact durations such as `20h`, `90m`, or `1d12h`, is capped per provider by `max_historical_days`, and can be disabled with `0m`. Re-fetched records use idempotent upserts, so unchanged values remain stable while late-finalised or revised records are refreshed. The cursor advances to the captured end only when every required provider substream succeeds, and it never moves backward if an older explicit window completes later; partial and failed pulls retry the same window next time. Historical pulls never change the live cursor, and `webhook` mode is unaffected.
</Note>
