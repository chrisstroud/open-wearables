...
1. `try_become_primary(provider, provider_user_id, user_id, scope="pull")` is called.
   - **Won** → captures one sync-window end, performs the provider work, and releases the lock. Only a fully successful pull advances the primary cursor and triggers `sync_vendor_data` for the other linked profiles with `_skip_linked_fan_out=True`; every fan-out receives the primary's exact effective start and end.
   - **Lost** → skips the API call and leaves `last_synced_at` unchanged while the lock holder finishes. The later fan-out advances this profile independently only if its own required provider work succeeds.

Partial or failed primary pulls do not fan out and do not advance any cursor. Successful cursor writes are monotonic, so an out-of-order fan-out with an older explicit window cannot regress a newer cursor. Historical fan-out preserves the same exact-window coordination but never changes the live cursor.
