...
| `items_processed` | `number \| null`      | Items in the run. For SDK uploads: samples and sleep records received, plus workouts newly inserted. See `metadata.inserted` / `metadata.updated` for rows actually written. |
