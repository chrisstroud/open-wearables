...
| Event | Description |
|-------|-------------|
| `connection.created` | A user successfully connected a wearable provider. For OAuth providers this fires when authorization completes; for SDK providers (Apple, Samsung) there is no OAuth callback, so it fires when the first data upload arrives. Also fires when a previously revoked SDK connection resumes uploading. |
| `connection.revoked` | A connection became invalid or was disconnected (refresh token expired/revoked, the user deregistered on the provider side, the user manually disconnected, or the connection stopped delivering data). The user must re-authorize to resume syncing. |
...
| `stale` | No successful sync within the configured window (`STALE_CONNECTION_DAYS`, default 14). Applies to SDK connections, which cannot report sign-out, revoked device permissions or app deletion. |
